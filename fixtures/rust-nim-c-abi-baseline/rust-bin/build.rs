...
//! --app:staticlib --noMain` and links it in — the "conventional C ABI
//! baseline" half of this fixture pair. Requires `nim` on `PATH` (see
...
fn main() {
    ...
    let status = Command::new("nim")
        ...
        .arg(&nim_src)
        .status()
        .unwrap_or_else(|e| panic!("failed to run `nim` (is it on PATH? see toolchains.lock.toml): {e}"));
    assert!(status.success(), "nim compilation of {} failed", nim_src.display());
