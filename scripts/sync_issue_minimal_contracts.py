...
CONTRACTS: dict[int, Contract] = {
    ...
    5: Contract("P1", "解決済みgraphから通常実行できるnative executableへのtarget routeを選択・生成できる。", "一つのowned objectと宣言済みforeign artifactを明示的にlinkし、native binaryを起動する。", "native routeの成立または不足IR/runtime/link obligationが判定できた時点。WASMやroute matrix完成は求めない。"),
    ...
    8: Contract("P0", "Cargo/Nimble/C/C++のdemandと制約を使えば候補直積を全展開せずnative executableに必要なclosureを選べる。", "各ecosystem一依存を含むtyped graphで整合要求1件と不適合1件を与え、探索・prune・merge・peak memoryをeager baselineと比較する。", "少なくとも一つのpruning/merge ruleの有効性または失敗が判定できた時点。"),
    9: Contract("P2 optional target", "解決済みgraphを任意のWasm target contractへ投影できる。", "native milestoneと分離し、最小の一方向依存を持つ2 unitで一つの統合方式と一つの拒否方式を比較する。", "WASM固有のsemantic/ABI factとtopologyを一つ判断できた時点。現在のnative goalをblockしない。"),
    ...
    16: Contract("P2 optional target baseline", "任意Wasm routeでlink/opt/component境界の必要性を識別できる。", "native milestoneと分離し、一つのCore Wasm artifactをopaque routeと一段展開routeで比較する。", "WASM固有の境界を一つ判断できた時点。現在のgoalやpipeline完成条件にはしない。"),
    ...
    22: Contract("P0/P1", "Cargo/Nimble/C/C++ closureでtoolchain version差をartifact/ABI互換性の明示制約として扱える。", "二つのtoolchain候補について一つのnative artifact reuse/link可否を比較し、不適合をcompile前に拒否する。", "一つの互換／非互換規則を証拠付きで決めた時点。全version matrixは求めない。"),
    ...
    43: Contract("P2 optional target", "owned Rust/Nim sourceから、必要byteとhost obligationを説明できる極小Wasm経路を作れる。", "native milestoneと分離し、既存R0をbaselineにsource-derived moduleのcapability delta一件を生成・実行する。", "WASM固有artifact差をsemantic/runtime obligationへ帰属できた時点。現在のgoalをblockしない。"),
    44: Contract("P0", "Cargo/Nimble graphから注入されたC/C++依存を独立artifact、ABI、symbol、link edgeとして解決しnative executableへ組み込める。", "一つのC libraryとadapterを要するC++ libraryをtyped closureへ接続し、一つの非互換ABIまたはsymbolをcompile前に拒否する。", "foreign dependency closureとhidden fallbackの境界を判断し、native binaryを起動できた時点。全C/C++統合は求めない。"),
}
...
現在のprimary artifactは通常実行できるnative binaryであり、WASMは任意targetである。単一call pathの成立をCargo/Nimble/C/C++ dependency closureの解決と同一視しない。
