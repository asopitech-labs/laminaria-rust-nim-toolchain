...
**限界**: (1) 到達可能性計算は1段の不動点(providerのproviderという多段連鎖は扱わない)であり、実際のfixtureの構造(全FFI要求のdeclaring_sourceがdemand自身)には十分だが、より深いFFI連鎖への一般化は未検証。(2) 測定はwall time(`Instant`)のみで、`laminaria_run::tracer`のpeak RSS計測はin-process呼び出しには不向き(プロセス全体のRSSを見ることになり、resolve単体のメモリコストを分離できない)ため未実施。

**Phase 2(native実行コスト)を比較workloadに含めることについて(issue #47直接受入基準の項目6、構造的事実による説明)**: 実際に合成providerへコンパイルコストを持たせてE0/E1の`cc`実行時間差を測ろうとしたところ、**LAMINARIAの既存実装が既にこの差を構造的にゼロにしている**ことが判明した。`resolve()`(E0)は`for candidate in &input.package_candidates { if selection.state != ObligationState::Selected { continue; } ... }`(1436行目付近)という設計により、`RequiredAction`(実際のcc呼び出しを表す)は**Selected状態の候補のみ**に対して生成される——未選択(到達不能)の合成provider候補は、resolve()自身が`resolve_provider_for_requirement`で候補プールから除外するため、`RequiredAction`には一切現れない。G2実行層(`compile_and_archive_cadd_v1`等)も、closureから該当するSelected obligationのみを対象にコンパイルを実行する設計であるため、**E0でもE1でも、実際に`cc`が起動される候補は到達可能なものだけで完全に一致する**。

これはissue #64の`bcm.c`(AWS-LCのunity build、118翻訳単位を1つの不可分な`cc`呼び出しへ強制結合)との決定的な違いを示す。`bcm.c`のPhase 1/2結合不可分性は、Cargo/`cc`という外部ツールが「翻訳単位」という粒度でしか到達可能性を扱えないことに起因する構造的欠陥だった。対してLAMINARIAの`ObligationState`モデルは、**候補(パッケージ)単位でPhase 1(Selected/Rejected判定)とPhase 2(RequiredAction生成・実行)を最初から分離している**——`resolve()`自体は一度もPhase 2の実行コスト(cc起動)を発生させない(単に`RequiredAction`という「実行すべき作業の記述」を生成するだけ)ため、E0/E1どちらであっても、Phase 2の無駄な実行は最初から起こり得ない。したがって「Phase-2重量級義務を比較workloadに含め、B-H3(summary/body分離)をそれに対し評価する」という項目6は、**この具体例(cadd/app fixture、パッケージ単位のcandidate粒度)においては、B-H3が問うていた問題自体がLAMINARIAの既存設計で既に解消されている**という形で結論づけられる。B-H3が本来必要になるのは、`bcm.c`のように**1つの候補(パッケージ)の内部が、外部ツールによってさらに独立したいくつもの翻訳単位に分割できる**場合(issue #64が実測した、118メンバー中98個が独立分解可能というケース)であり、そこでは「1候補=1RequiredAction」という現在の粒度モデル自体を、翻訳単位粒度まで細分化する必要がある——これは本issueのスコープ外(非ゴール「`cc`/CMakeをLAMINARIA独自のCコンパイラで置き換える実装」に接続する、より大きな設計課題)として、後続issueに委ねる。
