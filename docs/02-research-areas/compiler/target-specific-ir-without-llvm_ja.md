...
### 5.13.2 結果: 3回の独立実行全てで比率が1.0近傍に収束
...
3回とも比率は0.996〜1.026の範囲に収まり、測定誤差の範囲内で両者は同等の実行時性能を示した。

生成された機械語バイト列を比較すると、実は`target_ir`側の方が命令数が少ない: `target_ir`は`cmp; je; mov eax,imm; ret`(分岐後は即座に`eax`へ定数を入れて`ret`)のみだが、`opt-level=0`のLLVM側は`cmp; jne; mov [rsp-4],imm; jmp; mov [rsp-4],imm; mov eax,[rsp-4]; ret`と、スタックスロット経由の余分な往復を挟む(非最適化ゆえの冗長性であり、`-O`ビルドでは当然消える)。命令数で劣るはずのLLVM opt-level=0側と実測がほぼ同等だったという事実は、この規模の分岐一つでは分岐予測・パイプライン効果が支配的で、数命令の差は実行時間に有意な影響を与えないことを示唆する。

### 5.13.3 判定: 支持

節5.11.2の事前登録した支持条件「`opt-level=0`同士・同一呼び出し方式・N≥30試行で、本設計がLLVMと同等またはそれ以上の実行時性能を示す」は満たされた。統制条件下では、節5.8で観測された「本設計側がわずかに遅い」という結果や、節6に記録された「命令列が同一なのに性能差が生じる現象」は、この単純な分岐関数では再現しなかった。

ただし、これは限定的な支持であることを正直に記録する: 検証したのは`if param0 != 0 { 7 } else { 9 }`という最小の分岐一つのみであり、節5.8で扱った境界チェック(bounds-check)のようなより複雑な関数まで一般化できるかは未検証である。また、この結果は「本設計のコード生成がLLVMのopt-level=0出力と同程度に効率的」を意味するに過ぎず、「LLVMの最適化パス(opt-level=3)と同等」を意味するものではない。事前登録の棄却条件(「統制条件下でも原因不明の性能差が残る場合」)には該当しなかったため、事前登録の指示通りこれ以上の効率比較実験はここで打ち切る。
