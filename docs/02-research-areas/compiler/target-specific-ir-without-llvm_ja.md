...
| H6 | 8個の実targetそれぞれへの分岐可能性 | 意図的に打ち切り(節5.17)。x86_64/AArch64の2つは検証済みだが、残り6 targetへの汎用化はH3の結論(変換ロジックは共有できない)とユーザーの明示的指示(汎用化を追求しない)に基づき追跡しない |
...
## 5.17 H6(8 targetへの分岐)の打ち切りと、issue #68の完了判断

H6(8個の実targetそれぞれへの分岐可能性)の「威力偵察」として、まず最も構造的に異なるWASMを対象にするかを検討したが、これはユーザーから2点の指摘を受けて打ち切った。

1. **偵察の対象が既に確定済みの事実だった**: WASMが本設計のPC相対分岐モデル(x86_64/AArch64のCFGレイアウト+fixup機構)を再利用できないことは、issue #66-followupの時点で既に実データ確認済みだった(`lib.rs`のWASM調査: linkedモジュールは相対アドレス計算を伴うrelocationを持たず、`R_WASM_FUNCTION_INDEX_LEB`によるインデックス置換のみで、`ElfX86_64PendingReloc`の`offset`/`width`/`target`/`addend`という前提そのものが成立しない)。既に判明している結論をもう一度確かめる作業は偵察ではなく、何の新しい情報も生まない繰り返しだった。
2. **H6という仮説自体の性質**: H6は「target非依存の1つのIRを8target全部に汎用化できるか」という問いであり、H3が既に「`CfgBody`という表現は共有できるが、変換ロジックは共有できない」と判定した直後にこれを追いかけることは、「汎用化という幻想を捨てろ」という本セッション全体の明示的な指示に反する。H3の結果は「8番目のtargetを足しても、7個目までと同様に独立した変換ロジックが必要になるだけ」という結論を、既に強く示唆していた。

以上の理由により、H6はこれ以上の実験を行わず打ち切りとする。8 target中、x86_64とAArch64の2つ(H3・H4・H5・H2+H5統合で検証済み)を超えて残り6 target(Windows x86_64 MSVC/GNU、macOS ARM64、WASM 3variant)へ分岐する作業には着手しない。

issue #68全体としては、以下の理由により完了として扱う:

- H1(LLVM IR非経由の直接コード生成接続)、H4(CFG不動点計算モデル)、H5(`AddressState`統合)は支持で決着。
- H2(所有権情報の再エンコード)は`frozen`軸について支持(`unpin`軸は未着手だが、issueが求める中心的な問い「所有権情報を保持できるか」自体には実データで答えている)。
