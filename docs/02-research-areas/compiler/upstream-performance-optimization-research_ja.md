...
[独自コンパイラの責務契約](../../01-foundations/compiler-ownership-contract_ja.md)を研究目的と完了判定の基準とする。LLVM、MLIR、Polly、Pluto、Halide、ISPC、既存C/C++ compiler等はprior art、比較対象、実験用projectionとして扱う。これらへ本コンパイルを委譲することをLAMINARIAの成果とはしない。
