...
- **A0**: `BuildStrategyContext`判定（最尤ストラテジ/環境適応ストラテジ、issue #52で導入）。外部入力（ビルド呼び出しコンテキスト：CI設定ファイルの有無、直前のビルド履歴の有無）のみに依存し、他のいかなるアクティビティの出力も必要としない。**訂正（issue #83）**：以前「A1・N1はA0の後にのみ起動できる」と記述していたが、これはA0とA1/N1の間に実在しないデータ依存を演繹しただけの誤りだった——A1の入力（マニフェスト変更検知）・N1の入力（`.nimble`変更検知）はいずれもA0の出力（ストラテジ判定）を一切参照しない。A0が実際に消費されるのはA9（ターゲットパラメータ確定方法）のみであり、その手前のA1〜A8/N1〜N8には一切影響しない。issue #52本文が「戦略選択自体も段階1〜2(マニフェスト解析・依存解決)と同じタイミングで決定できる」と述べていたのは**並行に決定できる**という意味であり、順序を課す意図ではなかった。既存の依存レディ実行原則（`crates/laminaria-run/src/compiler_work_executor.rs`の`run_compiler_work_plan`：「外部入力のみに依存するアクションは起動条件が揃った時点で即座に着手できる」）に従えば、**A0・A1・N1はいずれも外部入力のみに依存する独立アクションであり、ビルド呼び出し開始時点で3つとも同時に着手できる**。A0が満たすべき唯一の制約は「A9が着手する時点までに完了していること」であり、A1・N1の前提条件ではない。
...
**N1の起動条件（issue #83で訂正）**：N1の入力（`.nimble`ファイルの変更検知）はA0の出力を一切参照しないため、N1はA0の完了を待つ必要がない——ビルド呼び出し開始時点でA0・A1・N1はいずれも外部入力のみに依存する独立アクションとして即座に同時着手できる。A1・N1とも「できるものは先にやっておく」という既存の依存レディ実行原則にそのまま従う。
...
## 状態保持レイヤーの設計原則：「コードのダブル」はIRレベルでのみ成立する（issue #67/#83/#84/#85）

issue #83（物理設計）でA9〜A10（コード生成）のデマンド駆動化を検討する過程で、「ソースコードとIRを並行する永続状態（分身）として持ち、変更差分だけを追従させられるか」という、issue #67が検証中の問い（問題F、`AddressState`の状態化+デマンド駆動化）が物理設計全体を貫く原則であることが判明した。

### Nimコンパイラ自身の実装史が示す実証結果：バックエンド出力レベルは失敗、意味解析済みIRレベルは成功

Nim公式リポジトリを一次資料として調査した結果、Nimコンパイラは過去に**2つの異なるレイヤーで「ダブル」を実装し、一方は放棄し一方はproduction化している**ことが判明した。

| | Nim旧`ccgmerge`（2021年に放棄） | Nim新IC（RFC #46、production化） |
|---|---|---|
| ダブルを持つレイヤー | バックエンド出力（生成済みCコード） | AST/意味解析済み中間表現 |
| 差分の反映方式 | Cファイルのold/newセクション部分マージ | モジュール単位でのAST全体replay |
| 結果 | 実装難易度が高く、新方式に完全に置き換えられ放棄 | 現行Nimの標準機能として実装・運用継続中 |

旧`ccgmerge`（`compiler/ccgmerge_unused.nim`という名前で現存、コード上どこからも参照されていないことを確認済み）は、生成済みCファイルをセクション単位で部分マージするという、バックエンド出力に近いレイヤーでの状態保持を試みていたが、2021年のPR #17311で新しいIC（Incremental Compilation、RFC #46）実装に完全に置き換えられ放棄された。新ICはモジュールのAST全体をSQLiteデータベースへ保存し、変更のあったモジュールを丸ごとreplayする方式であり、部分マージという低レベルの差分反映を採用していない。

**この対比が示す設計原則**：低レベル（コンパイル結果に近いほど）での差分保持は失敗し、高レベル（意味論を保持する中間表現）での状態保持の方が持続可能だった、という実証結果がNim自身の実装史に残っている。

### LAMINARIA自身の設計への反映

- issue #83が検討したA9〜A10のデマンド駆動化（「意味解析済み(A6/A7完了)だが機械語(A9〜A10出力)は未生成」という中間状態を保持する設計）は、Nim新ICと同じレイヤー（意味解析済みIR、`SemanticFact`/`InstantiationKey`相当）での状態保持であり、Nimの実装史が示す成功パターンと一致する。**バックエンド出力(LLVM IRや機械語)自体を差分マージする設計は検討しない**——それはNim旧`ccgmerge`と同じ失敗パターンを繰り返すことになる。
- issue #84が検討したNim側共有キャッシュストアも、当初`GeneratedCModule`（Cファイルレベル、旧`ccgmerge`と同じ粒度）での設計を検討していたが、この実証結果により`NimInstantiationKey`（意味解析済みIRレベル、宣言識別子・型引数・const generic値の3要素キー）でのキャッシュを優先すべきという結論を、より強い論拠で確定する。
- issue #85（耐久性分類の限界）が扱う「由来ベース分類のみでは不十分」という問題も、レイヤー選択の問題と独立に、観測ベースの判定機構が必要であることを示しており、IRレベルでの状態保持が「レイヤーを正しく選べば自動的にうまくいく」わけではないことも合わせて確認しておく必要がある。

いずれもissue #67の実現可能性検証が完了するまで、採用可否そのものは保留のままである——本セクションが確定するのは「採用する場合、どのレイヤーに置くべきか」という設計原則であり、「採用するかどうか」自体ではない。
...
- issue #52: Cargo/rustc/LLVMの実測分解、暫定ERD作成（クローズ済み）
- issue #71: エンティティモデル確定（10エンティティ、多重度、A0〜A12、Nim側対応表、foreign-native連鎖）（クローズ済み）
- issue #72: マクロ/テンプレート展開のサポート範囲確定（Rust/Nimとも現状は全面拒否、Nim側に明示的検出を実装）（クローズ済み）
- issue #73: `AdapterUnit`粒度確定（1:0..1）（クローズ済み）
- issue #74: ビルド全体フロー図統合、型・ABI突合のアドバイザリ化、突合粒度・失敗カテゴリ確定（クローズ済み）
- issue #75: FFI境界照合の型・ABIレベル拡張（arity検出）、Rust↔Nim新規照合実装（クローズ済み）
- issue #76: Nimビルドパスの実測分解、N1〜N9確定、巻き戻し検査（クローズ済み）
- issue #44: importc/importcpp/importobjcの実機検証（クローズ済み）
- issue #77: Nimbleの依存解決の実測（単一解検証は#79で補完達成、パッケージ名解決の構造的制約を発見）（クローズ済み）
- issue #78: `when defined(...)`条件グラフの実測確定（組み合わせ爆発なし、宣言数に線形）（クローズ済み）
- issue #79: Nimbleパッケージ間バージョン制約競合の実測（フェイルファスト、複数バージョン共存なし）（クローズ済み）
- issue #80: マクロ/テンプレート展開の具体的挿入点確定（N5に一体化、透過的前処理方式）（クローズ済み）
- issue #81: Nim重複コンパイルコストの規模別実測（小規模では無視できるが規模依存、実規模では秒オーダーに達する）（クローズ済み）
- issue #67: Rust IRを状態として持ち続けることは実現可能か——「コードのダブル」の実現可能性検証（進行中、状態保持レイヤーの設計原則の根拠issue）
- issue #82: Rust/Nim/foreign-native統合フローの書き込み競合・循環依存チェック（クローズ済み、起動順序とA11/N6の冪等性保証を確定）
- issue #83: A0起点(訂正: A0/A1/N1三者独立)の物理設計——データ量・実行時間・並行幅・CPU負荷削減の実測見積もり（進行中）
- issue #84: Nim側`NimBuildInvocation`境界を越えた共有キャッシュストアの設計（進行中、issue #67に従属）
- issue #85: 耐久性(durability)分類の限界——由来ベースのみでは不十分、観測ベースとのハイブリッド設計の検討（進行中）
