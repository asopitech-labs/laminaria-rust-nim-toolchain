...
本書は設計判断と13 caseの由来を保存するが、実装済み挙動の検証正本ではない。実装の正しさは、production実装を直接呼ぶ次の実行可能テストで判定する。

- `nim-planner/tests/test_incremental_kernel.nim`: case 1〜13のgraph状態、診断、generation、重複、supersession、需要管理をNim plannerへ直接適用する。
- `crates/laminaria-plan/src/incremental.rs`: Rust側wire contractのserialize/deserializeを直接検証する。
- `crates/laminaria-run/src/incremental_session_client.rs`: 実Nim binaryとのsession IPCとresponse identityを直接検証する。
- `crates/laminaria-run/src/incremental_executor.rs`: production executor、実compiler work、動的展開、CPU枠、計測、不正fallbackの否定を直接検証する。

旧 `issue-36-t0-cases.yaml`、fixture専用Python validator、そのvalidatorテストは、同一契約を三重に手動管理するだけで独立した正しさを与えなかったため廃止した。以下のYAML断片はwire formatの説明例であり、検証入力や正本ではない。
...
6. 旧case4の「重複」2 eventは実際には異なる`event_id`を持っており、
   ...
   分離する必要がある。
7. 旧case catalogのActionは`id`/`kind`/`inputs`/`outputs`のみで、契約が
...
catalog+具体的artifact ID、対象caseの修正)
...
Pythonで再実装し、実引数から計算した具体的な値であった。現在は
`crates/laminaria-plan/src/compiler_work.rs`のproduction関数に対する直接テストを正本とする。
...
`crates/laminaria-run/src/self_build.rs:1-27`と`docs/04-guides/self-build.md`が定義する
...
### 1.7 `docs/04-guides/self-build.md`の既存のpull型scheduling拒否への応答——設計の変化に合わせて再論証する

`docs/04-guides/self-build.md:66-78`は、Buck2/BazelのSkyframe型(依存未解決なら
...
`docs/01-foundations/research-foundations.md:268`の「material changesのみreplan」原則、
...
のいずれの`ActionStateChange.to_state`にも現れない。Nimのgraph状態と
Rustの実行status overlayを合成した観測は、production実装を呼ぶ直接テストで検証する(§10)。`ready→running`
...
`f_helper`という新規関数を呼ぶようになった版を、source-edit-triggered
supersessionの実演(§3.4)専用に追加した。現在の実行可能な正本は
`nim-planner/tests/test_incremental_kernel.nim`のsupersession suiteである。
...
case 1〜13の状態遷移と診断は
`nim-planner/tests/test_incremental_kernel.nim`がNim production kernelへ直接適用する。
実IR評価(`LowerSource`→`ValidateIr`→`EvaluateEvidence`、および
`DiscoverSourceDependencies`)と実行・計測の統合は
`crates/laminaria-run/src/incremental_executor.rs`の直接テストが担う。
case10のcycle検出も同じNim production kernelを直接呼び、外部compilerへ
fallbackしないことはexecutorのproduction sourceと実行経路に対するテストで確認する。
