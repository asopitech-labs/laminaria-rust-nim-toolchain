...
- `docs/05-history/issue-35-d0-cases.yaml`（schema_version `0.2.0-draft`、基準commit
  `c812d70`、`git log`で#35 D0受理(`0246514`)以降無変更を確認済み）
- `docs/03-work-items/design/issue-35-d0-d1-draft-instructions.md`（D1のスコープ・実装順序・
  検証方法・§6停止条件・§7未解決事項の扱い）
- `docs/03-work-items/design/issue-35-d0-spec.md` §10（採否確定記録、`issue35-d0-accepted-c812d70-v1`）
...
- `docs/03-work-items/design/d1b1-m1-m6-evidence.md`（M1/M6、隔離`c812d70` worktree手順）
...
```
loaded 20 cases from .../docs/05-history/issue-35-d0-cases.yaml
...
|---|---|---|---|---|---|
| M1-fingerprint-cold | Implemented, reference | `docs/03-work-items/design/d1b1-m1-m6-evidence.md`(手順記録)。実装コード追加なし(既存workspace実測) | ● 隔離`c812d70`worktreeでの1回のローカル実行記録(本セッション内、再現手順込み)。CI未自動化 | 適合(3/3回、`Compiling laminaria-fingerprint`exactly1) | CIで自動再確認されない。手順は固定commitに対して決定的なため再実行しても同一結果になるはずだが、独立した自動検証は存在しない |
...
| M6-diamond-fingerprint-plan | Implemented, reference | `docs/03-work-items/design/d1b1-m1-m6-evidence.md` | ● M1と同じ隔離worktree手順内で確認(本セッション内) | 適合(fingerprint/planとも収束点で重複コンパイルなし) | M1と同じCI未自動化 |
...
2. **M1-fingerprint-{cold,noop,leaf-edit}/M6-diamond-fingerprint-planは
   ...
   `docs/03-work-items/design/d1b1-m1-m6-evidence.md`に記録された、隔離`c812d70`
