...
## §1. 前提となる制約(`docs/01-foundations/compiler-ownership-contract.md`から直接引用)
...
| 独自bytecode + 自作VM | 実装の自由度が最大 | runtime契約を含め全てゼロから自作する必要があり、既存の外部検証手段(標準仕様に基づくruntime)を使えない。`docs/03-work-items/design/issue-35-d0-spec.md`のM10が要求するWASM経路を一切前進させない |
...
2. `docs/03-work-items/design/issue-35-d0-spec.md:63`(M10行)と`:284-291`(§3.3)が、
...
- `fixtures/laminaria-semantic-substrate-prototype`の2関数を超える
  fixtureを追加しない。
- `docs/03-work-items/design/issue-35-d0-spec.md`のM10(実際のWASM feasibility spike
