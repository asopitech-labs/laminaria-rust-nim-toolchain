...
- 併読ファイル:
  - [旧D0 case catalog](../../05-history/issue-35-d0-cases.yaml) — 採択時点のcase IDと未実装研究scopeを保存する履歴資料
...
| LSP | なし | — | 完全新規（M10で設計） |
| WASM | なし（`docs/02-research-areas/toolchains/agent-oriented-toolchain-ux.md:23`と`docs/01-foundations/metrics-policy.md`が将来のtarget経路として言及するのみで実装ゼロ） | — | 完全新規（M10で設計。実現可否は#5 T0の確定と独自target生成能力に依存し、本書はM10を「未達として保持」する選択肢を残す） |
...
stage_NN::run() -> u64 for NN >= 2 {
    transform[(NN - 1) % 3](../../design/stage_(NN-1)::run())
}
```
...
採択時点の機械可読な詳細は[旧D0 case catalog](../../05-history/issue-35-d0-cases.yaml)に履歴として保存する。本節はcase定義のschemaと横断的な設計判断を記すが、実装済み挙動の検証正本はproduction実装に対する直接テストである。
...
この一文を反映した箇所: `docs/03-work-items/design/issue-35-d0-d1-draft-instructions.md`「4. 期待結果」節、`docs/05-history/issue-35-d0-cases.yaml` M7-long-chain-wide-branches-smallの`subset_scope.d1_verifies`、本書§4.1のschemaコメント。
