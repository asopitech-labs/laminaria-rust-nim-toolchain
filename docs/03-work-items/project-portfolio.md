...
| First-class artifact test harness | Unit/integration tests and process evidence exist, but no production graph `TestContract` binds an exact subject artifact, harness, target environment, controls, observations, and raw result; the [Rust/C++ tool survey](../02-research-areas/toolchains/rust-cpp-testing-tool-landscape_ja.md) confirms that standard frameworks normally run separate test executables | Add lossless adapters for libtest/Cargo and CTest/framework discovery, while separately executing and inspecting the exact production artifact; model test demands/dependencies separately from release dependencies; derive incremental test selection and release qualification from the same graph | Lane C #49 for M1; later C3/C4 gaps remain unissued |
...
| Incrementality and resource evidence | Incremental planner, executor, tracing, reuse, and benchmark fixtures exist | Apply them to the same coupled graph and separate avoided work, cache reuse, invalidation, scheduling, and memory effects; first prove that Rust semantic/IR/link facts feed back into Cargo-compatible candidate/unit choice and eliminate work before execution, then expand only the Cargo/rustc/LLVM responsibility exposed by a counterexample | G3 #47; #6/#7/#11/#12/#19-#21; Rust closed-loop decision #50 |
