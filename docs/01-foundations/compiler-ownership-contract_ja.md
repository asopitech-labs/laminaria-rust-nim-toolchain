...
正準要件、graph model、vertical sliceの完了条件は[Nim C/C++ library統合](../02-research-areas/compiler/nim-c-cpp-library-integration_ja.md)に定める。
...
Windows、macOS、Linux/ラズパイの参加は、実行hostとコンパイルtarget、ISA/ABI、SDK/sysroot、runtime、target feature、信頼性を区別する。cross compileの同時実行は、生成nodeでtargetを実行・検証できることを意味しない。これらはIR・計算分割・永続化契約の設計条件である。[水平分散研究](../02-research-areas/execution/horizontal-distribution-research_ja.md)も参照。
