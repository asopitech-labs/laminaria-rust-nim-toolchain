...
**いずれの場合も**、RustとNimの両方が要求されている限り、検証済みの
Nimツールチェーンの `bin` ディレクトリは `CargoBuild`
アクション自身の `PATH` の先頭に追加される — `build.rs` が `nim`
を呼び出せるようにするためである。これは「Nimが要求されているかどうか」
(`req.rust && req.nim`) のみで決まり、別のNimエントリポイントが実在
するかどうかには依存しない: Nim *成果物* が生成されるかどうかと、Cargo
*アクション*がNimを必要とするかどうかは独立した事実である。以前の
実装では、この判定を `req.nim_entry.is_none()`
に基づいて行っていたためこの2つを混同しており、独立2プロデューサー形状で
`build.rs` が `nim` を呼び出すケースでは、本クレートが解決・検証した
はずのNimではなく、たまたまこのプロセス自身のPATH先頭にあった未固定の
`nim` が黙って使われてしまっていた。この点を修正し、実在するNim
エントリが存在する状態でも検証済みのbinディレクトリがPATHに載ることを
直接 `RootCommand` を構築して確認する専用テストでカバーしている。
...
終了コードが0であることだけを、要求された成果物が実際に存在する証拠とは
決して扱わない: 対象プロジェクト自身の `nim.cfg` が `--compileOnly:on`
を設定していると、`nim c` は要求された `-o:` 出力を一度もリンクせずに
終了コード0で終わる(実際に直接確認済み)。すべての候補成果物パスが
ディスク上に実在することを確認し、かつ少なくとも1つは存在しなければ、
そのアクションは成功として報告しない — 終了コードが0であってもこの検査に
失敗すれば失敗として報告し、記録済み `Run` 自身の `result.success`
もそれに合わせて訂正する。これにより、ディスク上の生の証拠が報告された
結果と食い違うことはない。

この存在確認は `is_file()` ではなく `exists()` を使う: Cargoの成果物は
正当に *ディレクトリ* であり得る — 例えば `[profile.release] debug =
true` かつ `split-debuginfo = "packed"` を設定したmacOSビルドの
`.dSYM` デバッグ情報バンドルで、これは実際にCargo自身の `filenames`
テレメトリに実行ファイルと並んで現れる(実際に確認済み)。`is_file()`
のみの検査ではこのディレクトリを拒否してしまい、本当に成功したビルドを
偽の失敗に変えてしまう — Cargo自身のテレメトリはそれが何を生成したかに
関して権威的な情報源であり、この検査はそれらのパスが実在することだけを
確認すればよい。
...
- 存在しないファイルを名指しした明示的な `--nim-entry`
  は一律拒否される — 純粋な推論経路でも、明示的な `--requires rust,nim`
  経路でも同様であり、「Nimエントリなし、Rustのみビルド」へ黙って
  ダウングレードされることはない。
- 実際のRust単独ビルドは、ビルド自身のPATHの先頭にセンチネルの
  `nim`/`nimble` を配置し、解決が試みられればPATH探索に落ちるよう仕向けた
  lockファイルを使っても、実際のplanner・実際の `cargo` によりエンドツー
  エンドで成功する — センチネルは一度も呼び出されない。Nim単独ビルドにも
  対称なテストがある: PATH先頭に置いたセンチネルの
  `rustc`/`cargo`/`rustup` も一度も呼び出されない。
- 実在するNimエントリが存在する状態で `--requires rust,nim`
  を指定しても、検証済みのNimツールチェーンがCargoアクション自身のPATHに
  載る — 別のNimエントリが存在しない場合だけでなく。
- `--compileOnly:on` を設定したNimの `nim.cfg` (要求された出力をリンク
  せずに終了コード0で終わることを実際に確認済み) は、存在しない成果物を
  名指しした偽の成功ではなく、ビルド失敗として報告される。記録済み
  `Run` 自身の `result.success` もそれに合わせて訂正される。
- `debug = true` かつ `split-debuginfo = "packed"` を設定したmacOSの
  Cargoビルド(これは本当に成功し、実行ファイルと並んで `.dSYM`
  *ディレクトリ*バンドルを報告する — 実際に確認済み)は成功のまま
  扱われる: 上記の成果物存在確認は、通常ファイルだけでなく実在する
  ディレクトリも受け入れなければならない。
- `plan-build` と `build` を同じカレントディレクトリから同じ相対
  `--project-root` で呼び出すと、同一の `plan_id` を計算する。
