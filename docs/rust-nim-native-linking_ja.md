...
**実証済みの知見(`fixtures/direct-native-link/NOTES.md`)**: fixed-layout recordは**ポインタ経由**であれば安全に共有できる — 独立に宣言したRustの`#[repr(C)] struct`とNimの`{.bycopy.} object`は、両者が実行時に自分自身のレイアウトを計算して相互比較する形で検証した結果、テスト済みの全Nim側ルート(`nim c`と`nlvm`)で一致した。一方、同じ構造体を**値渡し**(引数または返り値)で共有するのは一様に安全ではない — `nim c`ルートでは正しく動作するが、`nlvm`ではテストした全ての形状で壊れている(単独引数として渡すと静かに不正値、返り値として使うと静かに不正値、後続に他の引数がある場合はクラッシュ)。`nlvm`自身のコンパイラも返り値のケースを未完了のTODOとして自己申告している。したがって値渡しの集合体はバックエンドルートに依存し、この互換性matrixが採用するベースラインには現時点で含まれない。ポインタ経由が、テスト済みの全ルートで検証された安全なパターンであり、このプロジェクト自身のフィクスチャ群が独立に収束していた実践的デフォルトでもある。固定長整数・浮動小数点数・生ポインタ(Layer 1-2のスカラー)にはこのようなルート依存性は見られない。
