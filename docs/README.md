...
For an explicitly separate external/reference track, [Rust参照ビルド・チェーンの最小フットプリント研究ロードマップ](03-work-items/rust-reference-build-footprint-roadmap_ja.md) compares Cargo/rustc profiles by elapsed time, CPU, memory, disk, and I/O. It does not replace the owned compiler path or the current M1 gate.
