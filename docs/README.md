...
The [Rust build path（Cargo/rustc/LLVM置換）の省資源研究ロードマップ](03-work-items/rust-reference-build-footprint-roadmap_ja.md) makes resource minimization a first-class acceptance condition for the owned Rust path: Cargo-compatible resolution/unit construction, Rust semantic computation, and LLVM-equivalent optimization/target generation are replaced by LAMINARIA. Existing tools are reference/bootstrapping baselines only.
